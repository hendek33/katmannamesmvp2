...
export function GameCard({ card, onReveal, onVote, isSpymaster, disabled, voters = [], hasVoted = false, revealedImage, rowIndex = 0, isLastCard = false, isAssassinCard = false, gameEnded = false, isKnownCard = false }: GameCardProps) {
  ...
  return (
    <div
      ref={cardRef}
      className={cn(
        "relative rounded-lg border-[10px] p-0.5 sm:p-1 lg:p-1.5 w-full",
        "aspect-[3/2]",
        "flex flex-col",
    ...
    >
      ...
      {voters.length > 0 && !card.revealed && (
        <div className="absolute top-0.5 right-0.5 sm:top-1 sm:right-1 z-10 max-w-[70%]">
          <div className="flex flex-wrap gap-0.5 justify-end">
            {voters.map((voter, idx) => (
              <span
                key={voter}
                className={cn(
                  "px-0.5 py-0 sm:px-0.5 sm:py-0 md:px-1 md:py-0.5 lg:px-1 lg:py-0.5 xl:px-1.5 xl:py-0.5 text-[5px] sm:text-[6px] md:text-[7px] lg:text-[9px] xl:text-[10px] 2xl:text-[11px] rounded animate-pop-in bg-red-950/80 text-red-200",
