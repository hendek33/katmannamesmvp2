...
export function GameCard({ card, onReveal, isSpymaster, disabled }: GameCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "relative rounded-md border-2 p-1.5 md:p-2 w-full h-full max-h-full",
        "transition-all duration-300 flex flex-col overflow-hidden",
        colors.border,
        colors.bg,
        colors.shadow,
        canClick && "cursor-pointer hover:scale-[1.06] hover:-translate-y-1 active:scale-100",
    ...
    >
      ...
      <div className={cn(
        "rounded px-2 py-1 md:px-3 md:py-1.5 flex items-center justify-center",
        colors.panel
      )}>
